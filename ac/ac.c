...
#define Top_value (((long long)1<<Code_value_bits)-1)
...
static void update_model_adapt (ac_model *, int);
static void update_model_count (ac_model *, int);
static void update_model_bwt (ac_model *, int);
...
input_bit (ac_decoder *acd)
{
    ...
    static void
update_model_bwt (ac_model *acm, int sym)
{
    int i;
    if (acm->cfreq[0]>=997)  {
        int cum = 0;
    ...
    static void
update_model_count (ac_model *acm, int sym)
{
    int i;
    --acm->freq[sym];
    for (i=sym; i>=0; i--)
    ...
    static void
update_model_adapt (ac_model *acm, int sym)
{
    int i;
...
ac_model_init (ac_model *acm, int nsym, int *ifreq, int adapt)
{
    ...
    if (ifreq)  {
        ...
        if (adapt != MODEL_COUNT) {
            while (acm->cfreq[0] > Max_frequency) {
                int cum = 0;
                acm->cfreq[acm->nsym] = 0;
                for (i = acm->nsym-1; i>=0; i--)  {
                    acm->freq[i] = (acm->freq[i] + 1) / 2;
...
ac_encode_symbol (ac_encoder *ace, ac_model *acm, int sym)
{
    ...
    range = (long long)(ace->high-ace->low)+1;
    ace->high = ace->low + (range*acm->cfreq[sym])/acm->cfreq[0]-1;
    ace->low = ace->low + (range*acm->cfreq[sym+1])/acm->cfreq[0];

    for (;;)  {
        if (ace->high < Half)  {
            if (ace->high < ace->low) fprintf(stderr, "*** 0 ***\n"), exit(1);
            bit_plus_follow (ace, 0);
            if (ace->high < ace->low) fprintf(stderr, "*** 1 ***\n"), exit(1);
        }  else if (ace->low >= Half)  {
            bit_plus_follow (ace, 1);
            ace->low -= Half;
            ace->high -= Half;
            if (ace->high < ace->low) fprintf(stderr, "*** 2 ***\n"), abort();
        }  else if (ace->low>=First_qtr && ace->high<Third_qtr)  {
            ace->fbits += 1;
            ace->low -= First_qtr;
            ace->high -= First_qtr;
            if (ace->high < ace->low) fprintf(stderr, "*** 3 ***\n"), exit(1);
        }  else
            break;
        ace->low = 2*ace->low;
        ace->high = 2*ace->high+1;
        if (ace->high < ace->low) fprintf(stderr, "*** 4 ***\n"), exit(1);
    }

    if (acm->adapt == MODEL_ADAPT)
        update_model_adapt (acm, sym);
    else if (acm->adapt == MODEL_COUNT)
        update_model_count (acm, sym);
    else if (acm->adapt == MODEL_BWT)
        update_model_bwt (acm, sym);
...
ac_decode_symbol (ac_decoder *acd, ac_model *acm)
{
    ...
    range = (long long)(acd->high-acd->low)+1;
    cum = (((long long)(acd->value-acd->low)+1)*acm->cfreq[0]-1)/range;
...
void ac_encode_symbol_extralarge (ac_encoder *ace, ac_model *acm, int sym) {
    while (sym) {
        ac_encode_symbol(ace, acm, sym % AC_MAX);
        sym /= AC_MAX;
    }
    ac_encode_symbol(ace, acm, 0);
}

int ac_decode_symbol_extralarge (ac_decoder *acd, ac_model *acm) {
    int res = 0, mul = 1, x;
    while (0 < (x = ac_decode_symbol(acd, acm))) {
        res += x * mul; 
        mul *= AC_MAX;
    }
    return res;
